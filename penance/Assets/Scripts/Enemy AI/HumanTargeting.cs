...
public class HumanTargeting : MonoBehaviour
{
	public static int HumanDmg = 10;
	
	/*AI Targeting Fields*/  
	public float distance;
	public Transform target;
	public float attentionRadius = 30f;
	...
	void Start ()
	{
		...
		//bulletTracer = GetComponent<BulletTracer>();
	...
	void Update ()
	{
		...
		/*target distance is less than attention radius*/
		if (distance < attentionRadius) {
			/*Enemy is not attacking*/
			isItAttacking = false;
				
			/*color code for "has enemy's attention"*/
	...
	void lookAt ()
	{
		...
		{
			animation["agent_walk"].speed = 2;
			animation.CrossFade("agent_walk");
			Vector3 dir = (path.vectorPath[currentAIWaypoint]-transform.position).normalized;
        	dir *= searchSpeed;// * Time.deltaTime;
		...
		if (Vector3.Distance (target.position, transform.position) <= enemyAttackRange) {
			...
			if (direction > 0) {
				...
				if(random > .4)
				{
					/*Attack the player*/
					Debug.Log("HUMAN HIT");
					eh.AdjustCurrentHealth (-HumanTargeting.HumanDmg);
					animation["agent_shoot"].speed = 3;
					PlayerAttributes.Instance.hit = true;
					animation.CrossFade("agent_shoot");
	...
	void patrol ()
	{
		...
		if (moveDirection.magnitude < 0.5f) {
			if (curTime == 0) {
				...
				curTime = Time.time;
				animation.CrossFade("agent_idle");
			}
